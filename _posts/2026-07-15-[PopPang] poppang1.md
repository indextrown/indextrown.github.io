---
title: "팝팡에서 React Native 모듈로 iOS·Android 중복 개발을 줄인 과정"
tags:
  - React Native
  - iOS
  - Android
...
<figure>
  ...
  <figcaption>두 네이티브 앱을 유지하면서 공통 화면만 하나의 모듈로 개발했습니다.</figcaption>
</figure>

> iOS·Android 각 1인 개발 환경에서 같은 기능을 따로 만들던 비효율을 줄이고자, React Native를 양 플랫폼용 모듈로 빌드해 기존 앱에 연동하는 아이디어를 **AI로 빠르게 실험·적용**하고 공동 개발 환경 구축 및 생산성 약 2배 향상

## 한눈에 보기

| 구분 | 내용 |
| --- | --- |
| 문제 | iOS와 Android 개발자가 같은 화면과 기능 흐름을 각각 구현하고 있었습니다. |
...
| 결과 | 공통 화면의 구현 단위를 플랫폼별 2회에서 공통 모듈 1회로 줄여 해당 영역의 개발 생산성을 약 2배 높였습니다. |
...
## React Native로 바꿀 범위부터 정했습니다

React Native로 iOS와 Android 앱 전체를 다시 만들 생각은 없었습니다. 이미 안정적으로 동작하는 네이티브 앱은 유지하면서, 두 번 만들던 기능만 공통 코드로 개발하는 것이 목표였습니다.

기존 화면부터 옮기면 React Native 도입과 회귀 테스트를 동시에 진행해야 합니다. 우선 연동 방식을 빠르게 검증하고 기존 기능에 미치는 영향을 줄이기 위해, 새로 만드는 기능부터 React Native를 적용했습니다. 그중에서도 로그인 세션이나 앱 전역 내비게이션처럼 네이티브 앱이 관리하는 상태에 덜 의존하는 기능을 골랐습니다.
...
React Native 0.86의 Prebuild와 CocoaPods의 VFS 헤더는 팀에 익숙한 영역이 아니었습니다. `React.xcframework`가 어떤 헤더를 필요로 하는지, CocoaPods가 만든 설정 가운데 무엇이 Swift Package에 전달되지 않는지부터 확인해야 했습니다.

먼저 AI로 React Native 0.76 기반 사례와 0.86 소스를 비교해 `React.xcframework`가 Swift Package에서 동작하지 않는 원인 후보를 좁혔습니다. 빌드 스크립트와 헤더 변환 코드의 초안을 만들고, 오류가 날 때마다 다음 실험을 정하는 데도 AI를 활용했습니다.

다만 AI의 답을 바로 구현 근거로 삼지는 않았습니다. AI가 제안한 내용은 정답이 아니라 실제 빌드로 확인할 가설로 다뤘습니다.

설치된 Pod의 실제 디렉터리와 헤더를 살펴 가설이 맞는지 확인한 뒤, 시뮬레이터용과 디바이스용 XCFramework를 모두 만들었습니다. 마지막에는 Swift Package와 로컬 Maven 저장소를 네이티브 앱에 연결해 화면과 이벤트가 실제로 동작하는지 확인했습니다.
...
AI는 낯선 빌드 과정을 조사하고 실패 원인을 좁히는 시간을 줄여 줬습니다. 최종 판단만큼은 빌드 산출물과 iOS·Android 앱의 실제 동작을 보고 내렸습니다. 그럴듯하지만 틀린 설명을 구현 근거로 채택하지 않기 위해서였습니다.
...
팝업 제보와 팝업 제보 관리 화면은 [PopPang-RN](https://github.com/team-PopPang/PopPang-RN)에서 한 번 개발합니다. 두 모바일 개발자는 같은 데모 앱에서 화면과 동작을 함께 확인하고, 기능이 완성되면 릴리스된 모듈을 각 네이티브 앱에 연결합니다.

이전에는 공통 화면의 UI와 기능 흐름을 iOS와 Android에서 각각 구현했습니다. 지금은 공통 모듈에서 한 번 만든 코드를 두 모바일 개발자가 함께 수정하고 각 앱에 배포합니다.

물론 화면에 진입하는 방법, 앱 생명주기, 이벤트 처리, QA는 여전히 플랫폼별로 다뤄야 합니다. 따라서 전체 개발 기간이 항상 절반으로 줄어드는 것은 아닙니다.

공통 화면만 놓고 보면 구현 횟수가 플랫폼별 2회에서 공통 모듈 1회로 줄었습니다. 공통 화면을 한 번 만들면 두 앱에 쓸 수 있으므로, 이 영역의 생산성을 약 2배로 봤습니다.

처음부터 네이티브 코드를 없애려던 것은 아닙니다. 기존 앱과 개발 환경은 유지하면서 공통으로 만들 수 있는 기능만 한 번 개발하고 싶었습니다. AI는 이 과정에서 낯선 기술을 조사하고 실험하는 속도를 높여 줬습니다.

기술 전환의 범위는 필요한 화면으로 제한하면서도, 두 플랫폼이 하나의 기능을 함께 개발하고 배포할 수 있는 기반을 만들었습니다.

## 다음 목표는 직군과 관계없이 아이디어를 앱에서 검증하는 환경입니다
...
<figure>
  ...
  <figcaption>직군과 관계없이 아이디어를 공통 화면으로 만들고, 데모 앱에서 확인한 뒤 두 플랫폼에 함께 배포하는 환경을 구상하고 있습니다.</figcaption>
</figure>

React Native 공통 모듈의 역할을 iOS와 Android의 중복 구현을 줄이는 데서 끝내고 싶지는 않습니다. 다음 목표는 팝팡의 모든 팀원이 아이디어를 실제 화면으로 빠르게 검증할 수 있게 만드는 것입니다.
