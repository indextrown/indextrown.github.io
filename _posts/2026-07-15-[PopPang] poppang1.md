...
이론상 CocoaPods로 추가한 코드는 처음 한 번만 빌드되고, 증분 빌드 시에는 다시 빌드되지 않습니다. 하지만 알 수 없는 원인으로 다시 빌드되는 경우도 있습니다. Clean Build를 하면 다시 빌드 시간이 듭니다.

이 빌드 시간이 사라진다면 어떨까요? 예를 들어 1분이 절약되면, 하루 평균 checkout 횟수 × 개발자 수만큼 시간이 매일 절약됩니다. 평균 checkout 횟수가 5회이고 개발자가 10명이라면 하루 50분을 아낄 수 있습니다. 월 20일을 기준으로는 1,000분, 약 16시간입니다. 빌드가 2분 줄고 개발자가 20명이라면 한 달에 4,000분, 약 66시간을 아낄 수 있습니다.
