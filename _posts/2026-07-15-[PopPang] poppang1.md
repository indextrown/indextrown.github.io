...
title: "[PopPang] 네이티브로 개발하던 팝팡에 React Native를 더한 이유"
tags:
  - React Native
  - iOS
  - Android
...
React Native 프로젝트를 앱 개발의 중심에 두는 경우에는 자연스러운 구조입니다. 하지만 기존 iOS·Android 앱을 유지하면서 필요한 새 기능만 모듈로 추가하려던 팝팡의 요구와는 맞지 않았습니다. 선택하지 않은 이유는 두 가지였습니다.
...
이론상 CocoaPods로 추가한 코드는 처음 한 번만 빌드되고, 증분 빌드에서는 다시 빌드되지 않습니다. 하지만 원인을 찾기 어려운 재빌드가 발생할 수 있고, Clean Build를 하면 모든 의존성을 다시 빌드해야 합니다.

짧은 빌드 대기 시간도 checkout 횟수와 개발자 수만큼 쌓입니다. 빌드 시간이 1분 줄고 개발자 10명이 하루 평균 5회 checkout한다면 하루 50분을 아낄 수 있습니다. 월 20일을 기준으로는 1,000분, 약 16시간입니다. 빌드가 2분 줄고 개발자가 20명이라면 한 달에 4,000분, 약 66시간으로 늘어납니다.

같은 계산을 iOS 개발자 1명과 Android 개발자 1명인 팝팡에 대입했습니다. 하루 평균 checkout 5회, 월 20일을 기준으로 빌드 시간이 1분 줄면 `1분 × 5회 × 2명 × 20일 = 200분`, 즉 한 달에 3시간 20분을 아낄 수 있습니다. 2분이 줄면 `2분 × 5회 × 2명 × 20일 = 400분`으로, 한 달에 6시간 40분을 아낄 수 있습니다.

팀원들이 취업한 뒤 팝팡 개발에 쓸 수 있는 시간은 하루 최대 2시간이었습니다. 그래서 React Native 의존성을 각 네이티브 앱에서 반복해서 빌드하는 구조를 피하고 싶었습니다.
...
### 소스 코드에서 XCFramework와 AAR이 만들어지는 과정

<figure>
  <img src="{{ '/assets/img/2026-07-15-PopPang-RN/xcframework-aar-build-flow.svg' | relative_url }}" alt="PopPang-RN 소스가 iOS에서는 xcodebuild와 컴파일·링크를 거쳐 XCFramework로, Android에서는 Gradle과 컴파일을 거쳐 AAR과 Maven 저장소로 만들어진 뒤 기존 앱에 연결되는 흐름" width="100%">
  <figcaption>네이티브 소스의 컴파일과 패키징은 PopPang-RN에서 끝냅니다. 기존 앱은 완성된 XCFramework와 AAR을 받아 최종 앱 빌드에 연결합니다.</figcaption>
</figure>

빌드는 소스 코드를 앱에 포함할 수 있는 산출물로 바꾸는 전체 과정입니다. Xcode의 `xcodebuild`와 Gradle은 어떤 작업을 어떤 순서로 실행할지 관리합니다.

컴파일러는 소스 코드를 오브젝트 코드나 바이트코드로 바꾸고, 네이티브 링커는 컴파일된 코드의 심벌과 라이브러리를 연결해 바이너리를 만듭니다. 패키징 단계에서는 이 결과물에 헤더·Manifest·리소스·의존성 정보를 더합니다. PopPang-RN은 이 과정을 거친 iOS용 결과물을 XCFramework로, Android용 결과물을 AAR과 Maven 메타데이터로 묶었습니다.
...
[Apple 공식 문서](https://developer.apple.com/documentation/xcode/creating-a-multi-platform-binary-framework-bundle)에 따르면 XCFramework는 디바이스·시뮬레이터처럼 실행 환경이 다른 프레임워크·라이브러리 바이너리와 헤더를 하나로 묶는 형식입니다. PopPang-RN은 iOS 디바이스용과 시뮬레이터용 프레임워크를 각각 아카이브한 뒤 XCFramework로 묶습니다. Swift Package Manager는 이 결과물을 바이너리 타깃으로 받아 기존 iOS 앱에 연결합니다.
...
AI의 답변은 구현 근거가 아니라 실제 빌드로 확인할 가설로 다뤘습니다.

Pod 설치 디렉터리와 헤더를 직접 살펴 가설이 맞는지 확인했습니다. 그다음 시뮬레이터용과 디바이스용 XCFramework를 만들었습니다. 마지막에는 Swift Package와 로컬 Maven 저장소를 네이티브 앱에 연결해 화면과 이벤트가 실제로 동작하는지 검증했습니다.

| AI를 사용한 지점 | 직접 검증한 근거 |
| --- | --- |
| 공식 문서·오픈소스 비교, 버전 차이 요약 | React Native 0.86 Podspec과 설치된 Prebuild 산출물 확인 |
| VFS·헤더·모듈 문제의 원인 후보 도출 | 시뮬레이터·디바이스 아카이브와 Swift Package 빌드 |
| 빌드·릴리스 스크립트 초안과 오류 분석 | iOS·Android 앱 연동, 화면·이벤트 동작, v0.1.0 릴리스 |

AI를 활용해 낯선 빌드 과정을 조사하고 실패 원인을 좁히는 시간을 줄였습니다. 다만 최종 판단의 기준은 빌드 산출물과 iOS·Android 앱의 실제 동작이었습니다. 그럴듯한 설명보다 직접 확인한 결과를 우선했습니다.
...
공통 기능은 두 개발자가 함께 만들고, 네이티브 연동과 QA는 각 플랫폼 담당자가 맡습니다. 화면 진입 방식, 앱 생명주기, 이벤트 처리는 여전히 플랫폼별로 다뤄야 합니다. React Native를 도입했다고 해서 플랫폼별 작업이 모두 사라지거나 전체 개발 기간이 항상 절반으로 줄어드는 것은 아닙니다.
...
처음부터 네이티브 코드를 없애려던 것은 아닙니다. React Native의 적용 범위를 네이티브 상태에 덜 의존하는 새 기능으로 제한하고, 기존 앱과 개발 환경은 유지했습니다. AI는 그 과정에서 낯선 기술을 조사하고 실험하는 속도를 높이는 데 활용했습니다.

그 결과 두 모바일 개발자가 하나의 기능을 함께 만들고, 각자 담당하는 플랫폼에 배포할 수 있는 기반을 마련했습니다.
...
React Native 공통 모듈의 역할을 iOS와 Android의 중복 구현을 줄이는 데서 끝내고 싶지는 않습니다. 다음 목표는 팝팡의 모든 팀원이 아이디어를 실제 화면으로 빠르게 검증하는 환경을 만드는 것입니다.

백엔드 개발자·디자이너·모바일 개발자 누구나 [PopPang-RN](https://github.com/team-PopPang/PopPang-RN)을 열고 AI와 함께 React Native 화면을 만들 수 있는 구조를 목표로 합니다. 새로운 아이디어가 나오면 바이브 코딩으로 UI와 사용자 흐름을 구현하고 데모 앱에서 바로 실행합니다. 팀에서 검토를 마친 기능만 모듈로 릴리스해 iOS와 Android 앱에 함께 연결합니다.

먼저 공통 컴포넌트와 디자인 규칙을 정리합니다. AI가 프로젝트를 이해할 수 있도록 개발 지침과 구현 예시를 보강하고, 데모 앱에서 확인한 기능을 iOS·Android용 패키지로 배포하는 과정까지 자동화 범위를 넓힐 계획입니다.
