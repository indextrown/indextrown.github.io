...
blue > 이 글에서는 문제를 어떤 기준으로 원인을 좁혀갔으며, 불필요한 렌더링을 줄이기 위한 어떤 개선을 했는지 정리했습니다.
...
<div style="text-align: center;">
    <img src="{{ '/assets/img/2026-03-21-[SwiftUI] lifecycle/img.png' | relative_url }}" width="100%">
    <p style="text-align: center; font-size: 14px; color: gray; margin-top: 8px;">
...
2. 상태가 변경되면 해당 상태를 보유한 뷰와 그 자식 뷰 전체를 이전 스냅샷과 비교
    2-1. 만약 Equatable을 채택하고 있으면 개발자가 정의한 방식으로 뷰의 동일성을 파악 가능
    2-2. 변경된 것으로 감지된 뷰들은 invalidate(무효) 처리
3. body 호출
4. invalidate된 View들을중심으로 새로운 View 계층을 업데이트하고 렌더링을 다시 수행
